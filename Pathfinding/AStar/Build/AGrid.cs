...
{
    public LayerMask UnwalkableLayer = 0;
    ...
    public void CreateGrid()
    {
        ...
        Vector3 bottomLeft = Vector3.left * WorldSize.x * 0.5f - Vector3.up * WorldSize.y * 0.5f;
        Vector3 pivot;
        for(int i = 0; i< m_gridSizeX; ++i)
        {
            for(int j =0; j<m_gridSizeY; ++j)
            {
                pivot = bottomLeft + Vector3.right * (i * NodeRadius * 2 + NodeRadius) + Vector3.up * (j * NodeRadius * 2 + NodeRadius);
                pivot.z = 0;
                bool isWalkable = !Physics2D.OverlapBox(pivot, new Vector2(NodeRadius, NodeRadius), UnwalkableLayer);
    ...
    public void CreateGrid(string[] node)
    {
        ...
        for (int n = 1; n<node.Length; ++n)
        {
            ...
            Vector2 pos = new Vector2(float.Parse(value[3]), float.Parse(value[4]));
    ...
    void OnDrawGizmos()
    {
        ...
        if (Nodes != null)
        {
            foreach (ANode node in Nodes)
            {
                ...
                Gizmos.DrawCube(node.Position, Vector2.one * (NodeRadius * 2));
            }
        }
    }
#endif
