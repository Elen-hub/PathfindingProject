...
public class AStarBuildEditor : EditorWindow
{
    ...
    void SetGrid(ref Vector2 size)
    {
        ...
        m_grid.UnwalkableLayer = EditorGUI.LayerField(new Rect(size, uiSize), m_grid.UnwalkableLayer);
        size.y += 30;
        ...
        if (m_grid.WorldSize != Vector2.zero && m_grid.NodeRadius > 0.1f && m_grid.UnwalkableLayer != 0)
        {
    ...
    void BuildPathGrid()
    {
        ...
        int i = 1;
        nodes[0] = m_grid.UnwalkableLayer.value + "," + m_grid.NodeRadius + "," + m_grid.WorldSize.x + "," + m_grid.WorldSize.y;
